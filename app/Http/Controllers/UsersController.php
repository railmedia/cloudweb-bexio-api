...
class UsersController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('users.create_edit')
            ->with('item', $user)
            ->with('scopes', $user->getUserScopes())
            ->with('user_scopes', []);
    }
    ...
    public function store(Request $request)
    {
        ...
        if( ! $user->save() ) {
            ...
            if( $request->scopes ) {
                ...
                $meta->meta_key = 'bexio_scopes';
                $meta->meta_value = json_encode( $request->scopes );
                $meta->save();
    ...
    public function edit(Request $request, User $user)
    {
        return view('users.create_edit')
            ->with('item', $user)
            ->with('scopes', $user->getUserScopes())
            ->with('user_scopes', $user->user_scopes);
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        if( ! $user->save() ) {
            ...
            if( $request->scopes ) {
                ...
                $meta = UserMeta::where(['user_id' => $id, 'meta_key' => 'bexio_scopes'])->first();
                if( ! $meta ) {
                    $meta = new UserMeta;
                }
                $meta->user_id  = $id;
